...
def _planner_model(model: Any) -> Any:
    """Preserve planner loop behavior while allowing provider defaults for max_tokens."""
    try:
        if getattr(model, "_llm_type", "") == "coverity-assist":
            planner_max = os.getenv("COVERITY_ASSIST_PLANNER_MAX_TOKENS")
            explicit_max = None
            if planner_max not in (None, ""):
                explicit_max = int(str(planner_max).replace("_", ""))
